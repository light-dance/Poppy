...
async function writeEnvLocal(dbPort: number, redisPort: number): Promise<void> {
...
async function main() {
	...
	if (!dbContainer || !redisContainer) {
		console.error('\x1b[31m✗ Could not find db or redis container\x1b[0m')
	...
	await writeEnvLocal(dbPort, redisPort)

	console.log(`\x1b[32m✓ Docker verified (${projectContainers.length} containers)\x1b[0m`)
	console.log(`  db: localhost:${dbPort}, redis: localhost:${redisPort}`)
}
